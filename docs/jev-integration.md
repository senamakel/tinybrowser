...
guard, not a complete authorization system. Ambiguous labels such as
`Submit search` still pause because the snapshot does not expose a form action
or destination that could prove the click's effect.
